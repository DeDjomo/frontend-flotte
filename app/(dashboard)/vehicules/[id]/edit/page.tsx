// src/app/(dashboard)/vehicules/[id]/edit/page.tsx
'use client';
...
export default function EditVehiculePage() {
    ...
    const fetchVehicle = async () => {
        try {
            ...
            // Note: vehicle interface might not align perfectly with form data if naming differs
            // Adjust mapping as needed. Assuming standard names.
            setFormData({
                vehicleMake: vehicle.vehicleMake,
                vehicleType: vehicle.vehicleType,
                vehicleRegistrationNumber: vehicle.vehicleRegistrationNumber,
                vehicleName: vehicle.vehicleName || '', // Handle optional
                trackingDeviceId: (vehicle as any).trackingDeviceId || '', // trackingDetails might be nested or direct
                vehicleImage: null
            });

            // If vehicle has image URL, set preview
            if ((vehicle as any).vehicleImage) {
                setImagePreview((vehicle as any).vehicleImage);
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // trackingDeviceId might be complex to update if it's tied to external system, but we send it.
        if (!formData.trackingDeviceId.trim()) {
            // Optional check depending on requirements. nouveau requires it.
            // We keep it required for consistency
        ...
        try {
            ...
            await vehicleService.updateVehicle(id, {
                ...
                // trackingDeviceId might not be updatable via updateVehicle depending on API.
                // If it's part of UpdateVehicleDto, it's fine.
                // Based on vehicleService.ts, updateVehicle takes `UpdateVehicleDto`.
                // We include what we can.
    ...
    return (
        <div className={styles.container}>
            <p className={styles.breadcrumb}>
                ...
                {' '}/{' '}
                <span className={styles.breadcrumbCurrent}>Modifier véhicule</span>
            </p>
            ...
            <form onSubmit={handleSubmit} className={styles.form}>
                    ...
                    <label htmlFor="vehicleImage" className={styles.uploadLabel}>
                        {imagePreview ? (
                            ...
                            <>
                                ...
                                <span className={styles.uploadText}>Changer la photo</span>
                            </>
                        )}
                    </label>
                </div>
                ...
                <div className={styles.formActions}>
                    <Link href="/vehicules" className={styles.cancelButton}>
                        Annuler
                    </Link>
                    <button
                        type="submit"
